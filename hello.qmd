...
In our analysis, we run two ordinal regressions, one on strength, and one on online crack time. The strength variable is an ordered number 1-10, in order of increasing password strength, making ordinal a good fit. The strength variable meets the ordinal assumption of proportional odds, since it is reasonable to assume that one-unit changes in each predictor have the same conditional relationship with being in each strength category. For example, the strength variable is calculated in part based on password length, and each one character increase in password length has the same conditional relationship with being in each strength category. Additionally, for online crack time, we took the discrete values in seconds, and put them into categories 1-11 in terms of increasing strength (determined by increasing time taken to guess). This is ordinal as well since the variable true_val_strength is in increasing order of time taken to crack the password. The variable also meets the proportional odds assumption, as it is reasonable to assume that one-unit changes in each predictor have the same conditional relationship with being in each true_val_strength category. An example of this could be how each additional character in a password has the same conditional relationship with being in each strength category. 

*add to this -- add why true val is ordinal and also why*
...
End   = grep("AIC", sum2)
print(as.data.frame(unname(sum2[Start:End])), row.names=FALSE, quote = FALSE)
