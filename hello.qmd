...
*write out generic hypothesis test?* 

We use $\alpha = 0.05$ as our threshold for significance. 

Based on our strength model output, our significant predictors (i.e. the predictors with p-valeus less than 0.05) are `num_digits` and `num_unique`. This means that we have evidence to suggest that these two variables have a relationship with the log_odds of a password being classified as high or low-strength.  

In context, *interpret num_digits and num_unique* 

Based on our model output for online crack time, our significant predictors are `num_digits` and pass_length`. We have evidence to suggest that these two variables have a relationship with the log_odds of a password being classified as high or low-strength.  
